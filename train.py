...
def test(epoch, niter):
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        with open(
                test_loader.dataset.lines[batch_idx].replace('원천데이터', '라벨링데이터').replace(type, '.3D_json').replace('_NT',
                                                                                                                  '_TR').replace(
                        '.png', '.json').replace('\n', '')) as f:
            abcde = json.load(f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--datacfg', type=str, default='data/bookshelf1/bookshelf1.data')  # data config
    parser.add_argument('--modelcfg', type=str, default='cfg/yolo-pose.cfg')  # network config
    parser.add_argument('--initweightfile', type=str,
                        default='cfg/darknet19_448.conv.23')  # imagenet initialized weights
    ...
    bg_file_names = get_all_files('datasets/BG/JPEGImages')
    ...
    if use_cuda:
        model = torch.nn.DataParallel(model,
                                      device_ids=list(map(int, gpus.split(",")))).cuda()  # Multiple GPU parallelism
    ...
    for epoch in range(init_epoch, max_epochs):
        ...
        niter = train(epoch)
        # TEST and SAVE
        if ((epoch + 1) % 10 == 0) and ((epoch + 1) > 15):
            test(epoch, niter)
